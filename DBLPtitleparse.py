def extract_marked_lines(file_path):
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            for line in file:
                if line.startswith("#index"):
                    index_lines.append(line[len("#index"):].strip())
                elif line.startswith("#*"):
                    star_lines.append(line[len("#*"):].strip())
    ...
    return index_lines, star_lines

def write_to_output_file(output_file_path, index_lines, star_lines):
    try:
        with open(output_file_path, 'w', encoding='utf-8') as file:
            for index, title in zip(index_lines, star_lines):
...
index_lines, star_lines = extract_marked_lines(input_file_path)

write_to_output_file(output_file_path, index_lines, star_lines)